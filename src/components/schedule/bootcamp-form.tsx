...
export interface BootcampFormValues {
  subject_id: string
  tutor_id: string
  classroom_id: string
  ...
  start_time: string
  end_time: string
  selected_dates: string[] // YYYY-MM-DD
  student_ids: string[]
}
...
export function BootcampForm({ onSubmit, onCancel, isLoading }: BootcampFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="space-y-4 py-4">
        ...
        {/* Date Picker */}
        <div className="space-y-2">
          <Label>Bootcamp Dates * ({selectedDates.length} selected)</Label>
          <Popover>
            <PopoverTrigger asChild>
              ...
              >
                ...
                <span className="truncate">
                  {selectedDates.length > 0
                    ? formatDatesForDisplay(selectedDates)
                    : "Click to select bootcamp dates"}
                </span>
              </Button>
            </PopoverTrigger>
              ...
              <div className="space-y-2">
                {students.map((student) => (
                  <div key={student.id} className="flex items-center gap-2">
                    <Checkbox
                      id={`bootcamp-student-${student.id}`}
                      checked={selectedStudentIds.includes(student.id)}
                      onCheckedChange={() => toggleStudent(student.id)}
                    />
                    <label
                      htmlFor={`bootcamp-student-${student.id}`}
      ...
      <DialogFooter>
        ...
        <Button type="submit" disabled={isLoading || !isValid}>
          {isLoading ? "Creating..." : "Create Bootcamp"}
        </Button>
      </DialogFooter>
    </form>
  )
