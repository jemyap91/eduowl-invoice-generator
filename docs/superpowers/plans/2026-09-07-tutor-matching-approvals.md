...
- Function refusals use `ERRCODE '22023'`; missing rows use `P0002`; non-admin callers get `42501`. Messages, verbatim: "Session not found", "Only sessions waiting for approval can be edited here", "Enter the session date", "The date must stay in the submitted month", "Choose a rate tier", "rate tier does not belong to this assignment" (`23503`), "Hours must be more than 0", "Submission not found", "This submission is not waiting for approval", "No sessions to approve", "An invoice already exists for this month", "Give the tutor a reason".
...
- **Lock order is submission then entries, in every function.** `tm_approve_submission` locks `tm_submissions` then updates `tm_timesheet_entries`; `tm_edit_entry` now locks the entry's `tm_submissions` row first (when it has one) and only then the entry row, so the two functions can never deadlock and an edit that starts just before an approve serializes behind it instead of committing a stale total. Slice 5's delete-invoice function must follow the same order (lock the submission, then its entries) when it rolls a month back to `submitted`.
- **`tm_set_invoice_number` (slice 1) is not concurrency-safe.** It derives the next sequence number by counting existing rows for the month rather than using a sequence or an advisory lock, so two concurrent inserts for the same month can race to the same invoice number. This has not mattered because `tm_approve_submission` is the only writer today and it runs under the submission's row lock. When slice 5 adds more invoice writers (manual invoices, etc.), give `tm_set_invoice_number` a `pg_advisory_xact_lock` keyed on `(assignment_id, year, month)` or equivalent before it counts.
