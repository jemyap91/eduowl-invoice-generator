...
- Imported historical invoices are `source = 'manual'` (`scripts/import-master-list.ts`), so the dashboard's "Outstanding" tiles count every imported row as unpaid until an admin marks it, and "New manual invoice" for an imported month hits the "A manual invoice for this month already exists" message; both are correct behaviour the admin should expect.
